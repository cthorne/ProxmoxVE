...
DB_PATH="/usr/local/cthorne/filebrowser.db"
DEFAULT_PORT=8080
...
if [[ "${install_prompt,,}" =~ ^(y|yes)$ ]]; then
    ...
    mkdir -p /usr/local/cthorne
    chown root:root /usr/local/cthorne
    chmod 755 /usr/local/cthorne
    touch "$DB_PATH"
...
[Service]
User=root
WorkingDirectory=/usr/local/cthorne
ExecStartPre=/bin/touch /usr/local/cthorne/filebrowser.db
ExecStartPre=/usr/local/bin/filebrowser config set -a "0.0.0.0" -p ${PORT} -d /usr/local/cthorne/filebrowser.db
ExecStart=/usr/local/bin/filebrowser -r / -d /usr/local/cthorne/filebrowser.db -p ${PORT}
Restart=always
